...
def connect_ftp(host:str, port:int = FTP_PORT, usr:str = '', pwd:str = '', save_path:str = ''):
    connection = FTP()
    connection.connect(host=host, port=port, timeout=5)
    ...
    for file_info in l_file:
        ...
        find = file_name.endswith('.txt')
        find1 = file_name.endswith('.zip')
        if ((find == True) or (find1 == True)):
            print(file_name)
            save_file(connection, f'{actual_path}/{file_name}', f'{save_path}/{file_name}')
    connection.close()

if __name__ == '__main__':
    connect_ftp('modland.ziphoid.com', save_path='C:/Users/Fam.M/Desktop/TXT')
